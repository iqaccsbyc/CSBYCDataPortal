...
const NATURE_OPTIONS = ['Core Consultancy', 'Training and Organizational Development', 'Research Consultancy', 'Private Consultancy', 'Government Project', 'MOU Based', 'Any Other']
const ORG_IND_OPTIONS = ['Organization', 'Individual', 'Associated']
const STATUS_OPTIONS = ['Ongoing', 'On Hold', 'Completed']
const TYPE_OPTIONS = ['Sole', 'Collaborative', 'Private Consulting']
const SOURCE_OPTIONS = ['Faculty', 'CHRIST Consulting']
...
export default function ConsultancyEntry() {
  ...
  const initialForm = {
    id: null,
    natureOfConsultancy: 'Core Consultancy',
    title: '',
  ...
  useEffect(() => {
    ...
    const getMyName = async () => {
      ...
      if (!snap.empty) {
        setLoggedInFacName(snap.docs[0].data().facName)
      }
    }
    getMyName()
  }, [facId, user.email])
      ...
      if (loggedInFacName) {
        q = query(
          ...
          or(
            ...
            where('csbycCoFaculty', 'array-contains', loggedInFacName)
          )
        )
      } else {
        q = query(collection(db, 'consultancy'), where('submittedBy', '==', user.email))
      }
    }
  ...
  const handleInputChange = (e) => {
    ...
    setFormData(prev => {
      ...
      if (name === 'consultancyType' && value !== 'Collaborative') {
        updates.coFacultyList = ''
      }
      return { ...prev, ...updates }
    })
  }
  ...
  const handleAuthorsChange = (e) => {
    ...
    setFormData(prev => ({ ...prev, coFacultyList: value }))
  ...
  const handleSuggestionClick = (name) => {
    const parts = formData.coFacultyList.split(',')
    parts.pop() // remove the currently typed fragment
    const prefix = parts.length > 0 ? parts.join(',') + ', ' : ''
    setFormData(prev => ({ ...prev, coFacultyList: prefix + name + ', ' }))
    setShowSuggestions(false)
  }

  const handleSubmit = async (e) => {
    e.preventDefault()
    if (!formData.natureOfConsultancy || !formData.title || !formData.startDate || !formData.orgName) {
    ...
    try {
      const authorNames = formData.coFacultyList.split(',').map(s => s.trim()).filter(Boolean)
      ...
      const payload = {
        ...formData,
        csbycCoFaculty: identifiedFaculties,
      ...
      if (!formData.id) {
        ...
        await addDoc(collection(db, 'consultancy'), payload)
        setSuccessMsg('Consultancy entry submitted successfully!')
      } else {
        const docId = formData.id
        await updateDoc(doc(db, 'consultancy', docId), payload)
        setSuccessMsg('Consultancy entry updated successfully!')
      }
  ...
  const updateProofStatus = async (id, status) => {
    ...
    try {
      await updateDoc(doc(db, 'consultancy', id), {
  ...
  const handleDeleteEntry = async (entry) => {
    ...
    try {
      ...
      await deleteDoc(doc(db, 'consultancy', entry.id))
      setSuccessMsg('Consultancy entry deleted successfully.')
      setTimeout(() => setSuccessMsg(''), 5000)
    } catch (err) {
  ...
  const handleExport = () => {
    ...
    const exportData = filteredEntries.map((e, index) => ({
      ...
      'Nature of Consultancy': e.natureOfConsultancy || '-',
      'Project Title': e.title || '-',
      'Organization/Individual Type': e.orgType || '-',
      'Organization/Individual Name': e.orgName || '-',
      'Project Status': e.projectStatus || '-',
      'Start Date': e.startDate || '-',
      'End Date': e.endDate || '-',
      'Academic Year': e.academicYear || '-',
      'Consultancy Type': e.consultancyType || '-',
      'Co-Faculty List': e.coFacultyList || '-',
      'Source': e.source || '-',
      'Total Amount': e.totalAmount || 0,
      'Amount Received': e.amountReceived || 0,
    ...
    XLSX.utils.book_append_sheet(wb, ws, "Consultancy")
    XLSX.writeFile(wb, `Consultancy_Export_${new Date().toISOString().split('T')[0]}.xlsx`)
  }
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="mb-8">
        <h1 className="text-2xl font-bold text-gray-900">Consultancy</h1>
        <p className="text-sm text-gray-500 mt-1">Manage consultancy project records.</p>
      </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-10">
            ...
            <section>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Nature of Consultancy <span className="text-red-500">*</span></label>
                  <select name="natureOfConsultancy" value={formData.natureOfConsultancy} onChange={handleInputChange} required className="w-full rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-sm py-2 px-3 border">
                    {NATURE_OPTIONS.map(t => <option key={t} value={t}>{t}</option>)}
                  </select>
                </div>
                <div>
                ...
                <div className="md:col-span-2">
                  <label className="block text-sm font-medium text-gray-700 mb-1">Consultancy Project Name <span className="text-red-500">*</span></label>
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Organization/Individual <span className="text-red-500">*</span></label>
                  <select name="orgType" value={formData.orgType} onChange={handleInputChange} required className="w-full rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-sm py-2 px-3 border">
                    {ORG_IND_OPTIONS.map(t => <option key={t} value={t}>{t}</option>)}
                  </select>
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Organization/Individual Name <span className="text-red-500">*</span></label>
                  <input type="text" name="orgName" value={formData.orgName} onChange={handleInputChange} required className="w-full rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-sm py-2 px-3 border" />
                </div>
              </div>
            </section>
            ...
            {/* SECTION 2 */}
            <section>
              <h2 className="text-lg font-semibold text-gray-900 mb-4 border-b pb-2">Section 2: Team & Source</h2>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Type <span className="text-red-500">*</span></label>
                  <select name="consultancyType" value={formData.consultancyType} onChange={handleInputChange} required className="w-full rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-sm py-2 px-3 border">
                    {TYPE_OPTIONS.map(t => <option key={t} value={t}>{t}</option>)}
                  </select>
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Source <span className="text-red-500">*</span></label>
                  <select name="source" value={formData.source} onChange={handleInputChange} required className="w-full rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-sm py-2 px-3 border">
                    {SOURCE_OPTIONS.map(t => <option key={t} value={t}>{t}</option>)}
                  </select>
                </div>

                {formData.consultancyType === 'Collaborative' && (
                  <div className="md:col-span-2 relative">
                    <label className="block text-sm font-medium text-gray-700 mb-1">Co-Faculty Name(s)</label>
                    <p className="text-xs text-gray-500 mb-2">List all Co-faculty separated by comma (,). Start typing to select CS-BYC faculty.</p>
                    <textarea name="coFacultyList" value={formData.coFacultyList} onChange={handleAuthorsChange} rows={2} className="w-full rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-sm py-2 px-3 border" />
            ...
            {/* SECTION 3 */}
            <section>
              <h2 className="text-lg font-semibold text-gray-900 mb-4 border-b pb-2">Section 3: Timeline & Financials</h2>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Project Start Date <span className="text-red-500">*</span></label>
                ...
                <div>
                  ...
                  <input type="number" name="totalAmount" value={formData.totalAmount} onChange={handleInputChange} min="0" className="w-full rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-sm py-2 px-3 border" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Amount Received (₹)</label>
                  <input type="number" name="amountReceived" value={formData.amountReceived} onChange={handleInputChange} min="0" className="w-full rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-sm py-2 px-3 border" />
                </div>
              </div>
            </section>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="md:col-span-2">
                  <FileUploadField
                    ...
                    folderId={DRIVE_FOLDER_IDS.consultancy}
                    value={formData.proofLink}
                    onChange={(val) => setFormData(prev => ({ ...prev, proofLink: val }))}
                    filePrefix={formData.id || user?.email?.split('@')[0] || 'consult'}
                    fileType="proof"
                    allowLink={true}
                  />
                </div>
        ...
        {activeTab === 'entries' && (
          <div>
              <div className="mb-6 space-y-4">
                ...
                <div className="p-4 bg-gray-50 rounded-xl border border-gray-200 grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div>
            ...
            {loading ? (
              ...
              <div className="text-center p-8 bg-gray-50 rounded-xl border border-gray-200">
                <p className="text-gray-500">No consultancy entries found.</p>
              </div>
            ) : (
              <div className="overflow-x-auto rounded-xl border border-gray-200">
                <table className="min-w-full divide-y divide-gray-200">
                  <thead className="bg-gray-50">
                    <tr>
                      <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Consultancy Info</th>
                    ...
                    {filteredEntries.map(entry => (
                      <tr key={entry.id} className="hover:bg-gray-50 transition-colors">
                        <td className="px-6 py-4">
                          ...
                          <p className="text-xs text-gray-500 mt-1">{entry.natureOfConsultancy} · {entry.orgName}</p>
                          <p className="text-xs text-indigo-600 mt-1">Lead: {entry.submittedByName}</p>
                        </td>
                        <td className="px-6 py-4">
                          {entry.totalAmount && <p className="text-sm text-gray-900">Total: ₹{entry.totalAmount}</p>}
                          {entry.amountReceived && <p className="text-sm font-medium text-emerald-600">Received: ₹{entry.amountReceived}</p>}
                        </td>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-xl shadow-xl max-w-3xl w-full max-h-[90vh] overflow-y-auto">
            <div className="p-6 border-b border-gray-100 flex justify-between items-center sticky top-0 bg-white">
              <h3 className="text-xl font-bold text-gray-900">Consultancy Details</h3>
            ...
            <div className="p-6 space-y-6">
              <div className="grid grid-cols-2 gap-x-6 gap-y-4">
                ...
                <div><span className="block text-xs font-medium text-gray-500 uppercase tracking-wider">Nature of Consultancy</span><span className="mt-1 text-sm text-gray-900">{viewModalData.natureOfConsultancy}</span></div>
                ...
                <div><span className="block text-xs font-medium text-gray-500 uppercase tracking-wider">{viewModalData.orgType} Name</span><span className="mt-1 text-sm text-gray-900">{viewModalData.orgName}</span></div>
                <div><span className="block text-xs font-medium text-gray-500 uppercase tracking-wider">Type</span><span className="mt-1 text-sm text-gray-900">{viewModalData.consultancyType}</span></div>
                
                <div><span className="block text-xs font-medium text-gray-500 uppercase tracking-wider">Lead Faculty</span><span className="mt-1 text-sm text-gray-900">{viewModalData.submittedByName}</span></div>
                <div><span className="block text-xs font-medium text-gray-500 uppercase tracking-wider">Co-Faculty</span><span className="mt-1 text-sm text-gray-900">{viewModalData.coFacultyList || 'None'}</span></div>
                
                <div><span className="block text-xs font-medium text-gray-500 uppercase tracking-wider">Source</span><span className="mt-1 text-sm text-gray-900">{viewModalData.source}</span></div>
                <div><span className="block text-xs font-medium text-gray-500 uppercase tracking-wider">Total Amount</span><span className="mt-1 text-sm text-gray-900">{viewModalData.totalAmount ? `₹${viewModalData.totalAmount}` : 'N/A'}</span></div>
                
                <div><span className="block text-xs font-medium text-gray-500 uppercase tracking-wider">Amount Received</span><span className="mt-1 text-sm text-emerald-600 font-medium">{viewModalData.amountReceived ? `₹${viewModalData.amountReceived}` : 'N/A'}</span></div>
