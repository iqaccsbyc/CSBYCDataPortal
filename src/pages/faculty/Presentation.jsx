...
const ROLES = [
  'Invited Resource Person', 'Presenter', 'Participant', 'Session Chair',
  'Keynote Speaker', 'Panel Member', 'Workshop Facilitator'
]

const PROGRAMME_TYPES = [
  'International-level Conference', 'National-level Conference', 'State-level Conference',
  'International-level Workshop', 'National-level Workshop', 'State-level Workshop',
  'International-level Seminar', 'National-level Seminar', 'State-level Seminar',
  'FDP', 'QIP', 'Webinar', 'Invited Talk'
]

const NATURE_OF_PROGRAMME = ['Skill Training', 'Invited Talk', 'Awareness', 'Research', 'Academic']
...
export default function Presentation() {
  ...
  const initialForm = {
    id: null,
    date: '', programmeName: '', organizedBy: '', startDate: '', endDate: '',
    facName: '', facEmail: '', role: '', presentationTitle: '',
    programmeType: '', natureOfProgramme: '', level: '', mode: 'Physical', sponsoringBody: '',
  ...
  useEffect(() => {
    ...
    let q
    if (isElevated) {
      q = collection(db, 'presentations')
    } else {
      q = query(collection(db, 'presentations'), where('submittedBy', '==', user.email))
    }
  ...
  const handleInputChange = (e) => {
    ...
    setFormData(prev => {
      ...
      // Auto-suggest Level based on Type
      if (name === 'programmeType') {
        const t = value.toLowerCase()
        if (t.includes('international')) next.level = 'International'
        else if (t.includes('national')) next.level = 'National'
        else if (t.includes('state')) next.level = 'State'
      }
      return next
    })
  }
  ...
  const handleSubmit = async (e) => {
    ...
    // Conditionally check title
    const needsTitle = ['Presenter', 'Invited Resource Person', 'Keynote Speaker'].includes(formData.role)
    if (needsTitle && !formData.presentationTitle.trim()) {
      alert("Title of the Presentation is required for this role.")
      return
    }
    ...
    try {
      ...
      if (!formData.id) {
        ...
        await addDoc(collection(db, 'presentations'), payload)
        setSuccessMsg('Presentation entry submitted successfully!')
      } else {
        const docId = formData.id
        await updateDoc(doc(db, 'presentations', docId), payload)
        setSuccessMsg('Presentation entry updated successfully!')
      }
  ...
  const handleDeleteEntry = async (entry) => {
    ...
    try {
      ...
      await deleteDoc(doc(db, 'presentations', entry.id))
      setSuccessMsg('Presentation deleted successfully.')
      setTimeout(() => setSuccessMsg(''), 5000)
    } catch (err) {
  ...
  const handleExport = () => {
    ...
    const exportData = filteredEntries.map((e, index) => ({
      ...
      'Role': e.role || '-',
      'Presentation Title': e.presentationTitle || '-',
      'Programme Type': e.programmeType || '-',
      'Nature of Programme': e.natureOfProgramme || '-',
      'Level': e.level || '-',
      'Mode': e.mode || '-',
    ...
    XLSX.utils.book_append_sheet(wb, ws, "Presentations")
    XLSX.writeFile(wb, `Presentations_Export_${new Date().toISOString().split('T')[0]}.xlsx`)
  }
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="mb-8">
        ...
        <p className="text-sm text-gray-500 mt-1">Manage presentation records (papers/talks at external events).</p>
      </div>
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6 sm:p-8">
        ...
        {activeTab === 'new' && (
          <form onSubmit={handleSubmit} className="space-y-10">
            ...
            {/* SECTION 2 */}
            <section>
              <h2 className="text-lg font-semibold text-gray-900 mb-4 border-b pb-2">Section 2: Faculty & Role</h2>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                ...
                <div>
                  ...
                  <select name="role" value={formData.role} onChange={handleInputChange} required className="w-full rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-sm py-2 px-3 border">
                    <option value="">Select Role</option>
                    {ROLES.map(r => <option key={r} value={r}>{r}</option>)}
                  </select>
                </div>
                {['Presenter', 'Invited Resource Person', 'Keynote Speaker'].includes(formData.role) && (
                  <div className="md:col-span-2">
                    <label className="block text-sm font-medium text-gray-700 mb-1">Title of the Presentation <span className="text-red-500">*</span></label>
                    <textarea name="presentationTitle" value={formData.presentationTitle} onChange={handleInputChange} required rows={2} className="w-full rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-sm py-2 px-3 border" />
                  </div>
                )}
              </div>
            </section>
            ...
            <section>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Nature of the Programme <span className="text-red-500">*</span></label>
                  <select name="natureOfProgramme" value={formData.natureOfProgramme} onChange={handleInputChange} required className="w-full rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-sm py-2 px-3 border">
                    <option value="">Select Nature</option>
                    {NATURE_OF_PROGRAMME.map(n => <option key={n} value={n}>{n}</option>)}
                  </select>
                </div>
                <div>
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">Mode of Participation <span className="text-red-500">*</span></label>
                ...
                {(formData.programmeType?.includes('Conference') || formData.programmeType?.includes('Seminar')) && (
                  <div className="md:col-span-2">
                    <label className="block text-sm font-medium text-gray-700 mb-1">Sponsoring/Associating Professional Body</label>
                    <input type="text" name="sponsoringBody" value={formData.sponsoringBody} onChange={handleInputChange} placeholder="e.g. IEEE, ACM, Springer" className="w-full rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-sm py-2 px-3 border" />
                  </div>
                )}
              </div>
            </section>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="md:col-span-2">
                  <FileUploadField
                    ...
                    folderId={DRIVE_FOLDER_IDS.presentation}
                    value={formData.proofLink}
                    onChange={(val) => setFormData(prev => ({ ...prev, proofLink: val }))}
                    filePrefix={formData.id || user?.email?.split('@')[0] || 'pres'}
                    fileType="proof"
                    allowLink={true}
                  />
                </div>
            ...
            <div className="mb-6 space-y-4">
              ...
              <div className="p-4 bg-gray-50 rounded-xl border border-gray-200 grid grid-cols-1 md:grid-cols-4 gap-4">
                ...
                <div>
                  ...
                  <select value={filterRole} onChange={e => setFilterRole(e.target.value)} className="w-full text-sm rounded-md border-gray-300 py-1.5 px-3 border">
                ...
                <table className="min-w-full text-sm text-left">
                  <thead className="bg-gray-50">
                    <tr>
                      ...
                      <th className="py-3 px-4 font-semibold text-gray-500">Role</th>
                  ...
                  <tbody className="divide-y divide-gray-100">
                    {filteredEntries.map(e => (
                      <tr key={e.id} className="hover:bg-gray-50 transition-colors">
                        ...
                        <td className="py-3 px-4 text-gray-600">{e.role}</td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-3xl max-h-[90vh] flex flex-col">
            <div className="flex items-center justify-between p-6 border-b border-gray-100">
              <h3 className="text-lg font-bold text-gray-900">Presentation Details</h3>
            ...
            <div className="p-6 overflow-y-auto flex-1 space-y-6">
              <div>
                ...
                <div className="grid grid-cols-2 gap-4 text-sm">
                  ...
                  <div><span className="text-gray-500">Dates:</span> <p className="font-medium">{viewModalData.startDate} to {viewModalData.endDate}</p></div>
                </div>
              </div>

              <div>
                <h4 className="text-sm font-semibold text-gray-500 uppercase tracking-wider mb-3">Faculty & Role</h4>
                <div className="grid grid-cols-2 gap-4 text-sm">
                  <div><span className="text-gray-500">Faculty Name:</span> <p className="font-medium">{viewModalData.facName}</p></div>
                  <div><span className="text-gray-500">Role:</span> <p className="font-medium">{viewModalData.role}</p></div>
                  {viewModalData.presentationTitle && (
                    <div className="col-span-2"><span className="text-gray-500">Presentation Title:</span> <p className="font-medium">{viewModalData.presentationTitle}</p></div>
                  )}
                </div>
              </div>
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-4 text-sm">
                  ...
                  <div><span className="text-gray-500">Nature of Programme:</span> <p className="font-medium">{viewModalData.natureOfProgramme}</p></div>
                  ...
                  {viewModalData.sponsoringBody && (
                    <div className="col-span-2"><span className="text-gray-500">Sponsoring Body:</span> <p className="font-medium">{viewModalData.sponsoringBody}</p></div>
                  )}
                </div>
              </div>
